...
#Finitely presented version of the group of the chiral

fpiso:=IsomorphismFpGroup(q);
fp:=Image(fpiso);
#RelatorsOfFpGroup( fp ); #Here print the relations

#Permutation group version of the group of the chiral
piso:=IsomorphismPermGroup(Subgroup(q,[S1,S2]));
perm:=Image(piso);
V:=Image(piso,S2);
A:=Image(piso,P0*P1);
F:=Image(piso,S1);
